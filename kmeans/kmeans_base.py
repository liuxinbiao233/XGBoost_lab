...
class KMeansBase(object):
    ...
    def fit(self, dataset,index_h):
        ...
        for i in range(self.n_init):
            labels, centers, error = self._kmeans(dataset,index_h)
    ...
    # 合并fit和predict
    def fit_predict(self, dataset,index_h):
        self.fit(dataset,index_h)
        return self.predict(dataset)

    # kmeans的主要方法，完成一次聚类的过程
    def _kmeans(self, dataset,index_h):
        self.dataset = np.array(dataset)
        bestError = None
        bestCenters = None
        bestLabels = None
        centerShiftTotal = 0
        centers = self._init_centroids(dataset,index_h)
