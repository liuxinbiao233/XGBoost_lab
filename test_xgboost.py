...
from kmeans import kmeans_plus
...
class XGBoost(object):
    ...
    def __init__(self, n_estimators=10, learning_rate=0.001, min_samples_split=2,
                 min_impurity=1e-7, max_depth=2, is_regression=False, gamma=0., lambd=0.,random_state = None):
        self.n_estimators = n_estimators  # 树的数目
        self.learning_rate = learning_rate  # 训练过程中沿着负梯度走的步长，也就是学习率
        self.min_samples_split = min_samples_split  # 分割所需的最小样本数
        self.min_impurity = min_impurity  # 分割所需的最小纯度
    ...
    def fit(self, X, Y):
        ...
        for i in self.progressbar(range(self.n_estimators)):
            ...
            list_pred=h_backup
            max_h=max(list_pred)
            index_h=list_pred.index(min(list_pred))
            print("每一次迭代的最大误差:{}".format(max_h),"最小误差所在位置:{}".format(index_h))
            c=KMeansPlusPlus.k_means_plus_plus(self,X,index_h)
            center_data.append(c)
            zz=kmeans_plus.main(X,index_h)
            lunkuoxishu.append(zz)
        return center_data,lunkuoxishu
...
#
#     def __init__(self, n_estimators=10, learning_rate=1, min_samples_split=2,
...
class XGBClassifier(XGBoost):
    ...
    def __init__(self, n_estimators=30, learning_rate=0.36, min_samples_split=5,
                 min_impurity=1e-7, max_depth=2, is_regression=False,min_child_weight=2,
                 gamma=0., lambd=0.):
...
if __name__ == "__main__":
    ...
    #print(y_train)
    model = XGBClassifier()
    results_data=model.fit(X_train, y_train)
    y_pred = model.predict(X_test)
    score=model.score(X_test,y_test)
    print(score)   #验证识别精度
    res=np.array(results_data[1])
    res1=[]
    x1=[]
    for i in res:
        res1.append(i)
